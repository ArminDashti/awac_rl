...
class AdvEstimator(nn.Module):
    def __init__(self, actor, critics, cfg, popart=False, method="mean", ensembling="mean", n=4):
        super().__init__()
        self.cfg = cfg
        self.actor = actor
...
class AdvEstimatorFilter(nn.Module):
    def __init__(self, adv_estimator, cfg, filter_type="binary", beta=1.0):
        super().__init__()
        self.cfg = cfg
    ...
    def forward(self, state, action, step_num=None):
        adv = self.adv_estimator(state, action)
